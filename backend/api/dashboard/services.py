...
dashboard_URL = "https://sheetdb.io/api/v1/ma8opp1ci2oqd"
...
def get_dashboard_info_admin(request):
    ...
    if response.status_code == 200:
        ...
        for item in data:
            cursor.execute(
                "INSERT INTO dashboards (msv, stt, first_name,last_name,class,Go_to_the_board,Summarize_Mindmap,code_sytem,Total) VALUES (?, ?, ?,?, ?, ?,?, ?, ?)",
    ...
    if response.status_code == 200:
        ...
        if student_data:
            ...
            for item in student_data:
                ...
                cursor.execute("SELECT * FROM dashboards WHERE msv = ?", (item['Mã sinh viên'],))
                exists = cursor.fetchone()[0]

                if exists:
                    # Cập nhật bản ghi nếu đã tồn tại
                    cursor.execute(
                        "UPDATE dashboards SET stt = ?, first_name = ?,last_name = ?,class = ?,Go_to_the_board = ?,Summarize_Mindmap = ?,code_sytem = ?,Total = ? WHERE msv = ?",
